...
- **Build System**: 
    - Created a robust `CMakeLists.txt` with automatic backend detection and protocol generation via `wayland-scanner`.
...
- **Vulkan Backend**:
    - Vulkan 1.3 Instance and Surface creation (Wayland & X11).
...
- **Input Refinement**: 
    ...
    - Implemented full native pointer support (position, delta, buttons) for both Wayland and X11.
