 ...
 */
static inline sbgl_Camera sbgl_CameraPerspective(float fov_y_rad, float aspect, float near_p, float far_p) {
    sbgl_Camera cam = {0};
 ...
 */
static inline sbgl_Camera sbgl_CameraOrthographic(float left, float right, float bottom, float top, float near_p, float far_p) {
    sbgl_Camera cam = {0};
...
static inline sbgl_Mat4 sbgl_CameraGetProjection(const sbgl_Camera* cam) {
    ...
    return sbgl_Mat4Orthographic(cam->ortho_left, cam->ortho_right, cam->ortho_bottom, cam->ortho_top, cam->near_plane, cam->far_plane);
}
 ...
 */
static inline void sbgl_RaySphereIntersectBatch(sbgl_Ray ray, const sbgl_Sphere* spheres, sbgl_HitResult* results, uint32_t count) {
    for (uint32_t i = 0; i < count; ++i) {
        ...
        if (discriminant >= 0) {
            ...
            if (t > 0) {
                ...
                results[i].normal = sbgl_Vec3Normalize(sbgl_Vec3Sub(results[i].point, spheres[i].center));
            }
        }
    }
}
 ...
 */
static inline void sbgl_RayAABBIntersectBatch(sbgl_Ray ray, const sbgl_AABB* boxes, sbgl_HitResult* results, uint32_t count) {
    sbgl_Vec3 inv_dir = { { 1.0f / ray.direction.x, 1.0f / ray.direction.y, 1.0f / ray.direction.z, 0.0f } };
    ...
    for (uint32_t i = 0; i < count; ++i) {
        ...
        if (results[i].hit) {
            ...
            results[i].point = sbgl_Vec3Add(ray.origin, sbgl_Vec3Mul(ray.direction, results[i].distance));
            ...
            if (fabsf(p.x) >= fabsf(d.x) - epsilon) results[i].normal.x = p.x > 0 ? 1.0f : -1.0f;
            else if (fabsf(p.y) >= fabsf(d.y) - epsilon) results[i].normal.y = p.y > 0 ? 1.0f : -1.0f;
            else if (fabsf(p.z) >= fabsf(d.z) - epsilon) results[i].normal.z = p.z > 0 ? 1.0f : -1.0f;
        }
    }
}
