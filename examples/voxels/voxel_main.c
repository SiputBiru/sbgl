	...
	for (int z = 0; z < VOXEL_WORLD_SIZE; z++) {
		for (int x = 0; x < VOXEL_WORLD_SIZE; x++) {
			...
			for (int oct = 0; oct < 6; oct++) {
				...
				// Use consistent wrapping across octaves to match the 2048x2048 world size.
				// stb_perlin_noise3 wraps internally at 256. For a 2048 world,
				// we ensure the noise period divides 2048 evenly.
