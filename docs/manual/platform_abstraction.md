...
*   **Win32:** Triggered when the `WindowProc` intercepts a `WM_CLOSE` message. This typically occurs when the "X" button is clicked or `Alt+F4` is pressed.
...
Once `sbgl_WindowShouldClose` returns `true`, it is the application's responsibility to:
*   Exit the main execution loop.
*   Call `sbgl_Shutdown(ctx)` to perform a graceful cleanup of all native handles and memory arenas.
