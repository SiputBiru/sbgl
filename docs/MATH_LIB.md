# SBgl Math Library Architecture

SBgl includes a math library designed for Data-Oriented Design (DOD) workflows. The library is contained within a single header file (sbgl_math.h) and utilizes `static inline` functions for compiler inlining.
...
*   **Padded Vectors**: sbgl_Vec3 is explicitly padded to 16 bytes. While this increases memory footprint by 4 bytes per vector, it ensures that all 3D data is 128-bit aligned. This is a deliberate trade-off to enable fast SIMD loads, ensure perfect cache line alignment in arrays, and maintain direct compatibility with GPU buffer layouts (std140/std430).
...
### Constructors
The library provides `static inline` constructor functions to simplify type initialization and ensure correct padding.
*   sbgl_vec2(), sbgl_vec3(), sbgl_vec4(), sbgl_quat().
...
### Vector Operations
```c
sbgl_Vec3 position = sbgl_vec3(1.0f, 2.0f, 3.0f);
sbgl_Vec3 velocity = sbgl_vec3(0.0f, 1.0f, 0.0f);
...
sbgl_Mat4 model = sbgl_Mat4Translate(sbgl_vec3(10.0f, 0.0f, 0.0f));
...
sbgl_Quat rotation = sbgl_QuatFromAxisAngle(sbgl_vec3(0.0f, 1.0f, 0.0f), SBGL_PI / 2.0f);
